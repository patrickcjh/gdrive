...
func (self *Drive) listAllFiles(args listAllFilesArgs) ([]*drive.File, error) {
	...
	err := self.service.Files.List().Corpora("user,allTeamDrives").SupportsTeamDrives(true).IncludeTeamDriveItems(true).Q(args.query).Fields(args.fields...).OrderBy(args.sortOrder).PageSize(pageSize).Pages(context.TODO(), func(fl *drive.FileList) error {
